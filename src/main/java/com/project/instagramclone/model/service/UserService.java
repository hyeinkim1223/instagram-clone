...
public class UserService {
    ...
    public boolean 이메일중복체크기능(String email) {
        ...
        // 돌려보내기
        // 이메일 중복체크를 할 때 숫자로 시작하거나 특정 이메일로 시작하는 이메일은
        // 우리 회사에 가입을 못하게 하였으므로 중복체크 자체가 안된다.
    ...
    public boolean 회원가입(User user) {
        ...
        이메일 중복 체크 자체를 핸드폰 번호 인증을 했는가 먼저 본다음에 이메일 중복체크를 진행
    ...
    /**
     * 카카오 소셜 로그인 전용 회원가입
     * - SNS 회원가입 할 때 email 은 인증번호 발송 검증 건너뛰고, SNS Social 측에서 인증 대신
    ...
    public void 카카오회원가입(User user) {
        ...
        // 이메일중복체크기능의 경우 유저가 프론트엔드부터 작성해놓은 유저정보에서 email 을 어떻게 가입해서
